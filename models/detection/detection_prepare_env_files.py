...
def main():
    ...
    for fold in range(1):
        out_file = "config/environment_summit_fold" + str(fold) + ".json"
        env_dict = {}
        env_dict["model_path"] = os.path.join(out_trained_models_dir, "model_summit_fold" + str(fold) + ".pt")
        env_dict["data_base_dir"] = resampled_data_base_dir
        env_dict["data_list_file_path"] = os.path.join(downloaded_datasplit_dir, "dataset_fold" + str(fold) + ".json")
        env_dict["tfevent_path"] = os.path.join(out_tensorboard_events_dir, "summit_fold" + str(fold))
        env_dict["result_list_file_path"] = os.path.join(
            out_inference_result_dir, "result_summit_fold" + str(fold) + ".json"
        )
        with open(out_file, "w") as outfile:
            json.dump(env_dict, outfile, indent=4)

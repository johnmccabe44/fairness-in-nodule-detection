...
def setup_logging():
    logger = logging.getLogger('ticnet-logger')
...
def parse_logs(logs, logger):

    for log in logs:
        log_record = logging.LogRecord('ticnet-logger',
                                    level=log['level'],
                                    pathname=__file__,
                                    lineno=0,
                                    msg=log['msg'],
                                    args=(),
                                    exc_info=None)
        log_record.created = log['created']
...
def preprocess(idx, scan_ids, scans_root, mask_root, save_dir, overwrite, do_resample):

    try:

        scan_id = scan_ids[idx]
        study_id = scan_ids[idx].split('_')[0]
        ...
        if not overwrite and os.path.exists(os.path.join(save_dir, '%s.npy' % (scan_id))):
            return {'level': logging.INFO, 'created': time.time(), 'msg': f'{scan_id} already processed'}

        lung_mask, _, _ = load_itk_image(mask_path)
        img, origin, spacing = load_itk_image(scan_path)
        ...
        np.save(os.path.join(save_dir, '%s_origin.npy' % (scan_id)), origin)
        np.save(os.path.join(save_dir, '%s_spacing.npy' % (scan_id)), spacing)
        np.save(os.path.join(save_dir, '%s_ebox.npy' % (scan_id)), np.array((z_min, y_min, x_min)))
        np.save(os.path.join(save_dir, '%s.npy' % (scan_id)), seg_img)

        return {'level': logging.INFO, 'created': time.time(), 'msg': f'{scan_id} processed'}
    
    except Exception as e:

        return {'level': logging.ERROR, 'created': time.time(), 'msg': f'{scan_id} failed: {e}'}
...
def main():
    ...
    if args.batch_size != -1:
        scan_ids = scan_ids[args.batch_size * (args.batch_number - 1):args.batch_size * args.batch_number]
    ...
    if args.workers > 1:
        ...
        with Pool(args.workers) as p:
            logs = p.map(partial_preprocess, range(N))
